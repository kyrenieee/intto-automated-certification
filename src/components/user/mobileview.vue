...
const departments = ['CITCS', 'CAS', 'CCJE', 'CTE', 'CHTM', 'CAFA', 'CDE', 'COA', 'CON', 'COL', 'CBA']
  ...
  <main class="min-h-screen bg-[#1A2621] text-white font-poppins flex flex-col mx-auto relative max-w-md shadow-2xl overflow-x-hidden">
    ...
    <div v-if="currentStep === 0" class="flex flex-col h-full min-h-screen">
      <div class="flex-1 flex flex-col items-center justify-center p-8 text-center pt-16">
        ...
        <h1 class="text-xl font-semibold tracking-wide">Technopreneurship Demo Day 7</h1>
    ...
    <div v-else-if="currentStep >= 1 && currentStep <= 3" class="flex flex-col flex-1 p-6 min-h-screen">
      ...
      <div v-if="currentStep === 2" class="flex flex-col gap-6 flex-1">
        <div>
          <label class="block text-xs text-gray-300 mb-2 pl-1">Which of the following apply to you?</label>
          <select 
          ...
          >
            ...
            <option v-for="role in roles" :key="role" :value="role" class="bg-[#1A2621]">{{ role }}</option>
          </select>
        </div>
        
        <div>
          <label class="block text-xs text-gray-300 mb-2 pl-1">From which department are you from?</label>
          <select 
            v-model="store.formData.department"
            class="w-full bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.15)] rounded-full px-5 py-3 text-sm focus:outline-none appearance-none cursor-pointer"
          >
            <option value="" disabled class="bg-[#1A2621]">Select Department</option>
            <option v-for="dept in departments" :key="dept" :value="dept" class="bg-[#1A2621]">{{ dept }}</option>
          </select>
        </div>

        <div>
          <label class="block text-xs text-gray-300 mb-2 pl-1">What is your age?</label>
        ...
        <div>
          <label class="block text-xs text-gray-300 mb-2 pl-1">What would you rate this event?</label>
          <div class="w-full bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.15)] rounded-full px-5 py-3 flex justify-around items-center">
            <button 
              v-for="star in 5" 
              :key="star"
            ...
            >
              <svg 
                class="w-6 h-6" 
                :class="store.formData.rating >= star ? 'text-[#F4C150] fill-[#F4C150]' : 'text-gray-500 fill-transparent'"
                stroke="currentColor" 
                viewBox="0 0 24 24" 
                stroke-width="1.5"
              >
