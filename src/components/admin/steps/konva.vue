...
let stage = null, layer = null, bgRect = null, bgImageNode = null, templateImg = null, resizeObserver = null
...
const fitCover = (imgW, imgH, boxW, boxH) => {
  ...
  let width, height
  if (imgRatio > boxRatio) { height = boxH; width = boxH * imgRatio }
  else { width = boxW; height = boxW / imgRatio }
  return { width, height, x: (boxW - width) / 2, y: (boxH - height) / 2 }
}

const getStageSize = () => ({ width: containerEl.value?.clientWidth || 600, height: containerEl.value?.clientHeight || 375 })
...
const createTextNode = (key) => {
  const entry = props.variableMap[key]
  const node = markRaw(new Konva.Text({
...
const destroyTextNode = (key) => {
  ...
  if (node) { node.destroy(); textNodes.delete(key) }
}
...
const initStage = () => {
  ...
  stage = markRaw(
    new Konva.Stage({
     container: containerEl.value, width, height }
    )
  )
...
onMounted(async () => {
  ...
  resizeObserver = new ResizeObserver(() => {
    ...
    positionBackground()
    for (
      const [key, node] of textNodes.entries()) 
      positionNode(key, node)
    layer.batchDraw()
  })
