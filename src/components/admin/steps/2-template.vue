<template>
  ...
  >
    ...
    >
      <div>
        ...
        <p class="text-sm sm:text-base font-normal text-[#d3d3d3] mt-2">
          Upload your certificate template design. PNG, JPG, and PDF are
          supported.
        </p>
      </div>
    ...
    >
      ...
      <div
        v-if="!eventForm.templateFile"
        class="flex flex-col items-center text-center px-6"
      >
        <svg
          class="w-12 h-12 text-[#d1d5dc] mb-3 group-hover:text-white transition-colors duration-200"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
        ...
        <span
          class="text-[#d1d5dc] text-lg font-medium mb-4 group-hover:text-white transition-colors duration-200"
        >
          Upload your certificate template
        </span>

        <div
          class="bg-[#3b82f6] hover:bg-blue-600 active:bg-blue-700 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 font-medium shadow-md transition-colors duration-150"
        >
          <svg
            class="w-4 h-4"
            fill="none"
      ...
      <div
        v-else
        class="flex flex-col items-center text-center px-6 w-full"
        @click.prevent
      >
        <img
          v-if="previewUrl"
        ...
        <div
          v-else-if="eventForm.templateUrl"
        ...
        <p v-if="uploadError" class="text-xs text-red-300 mt-2">
          ...
          <button
            type="button"
            @click.stop.prevent="retryUpload"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const setFile = (file) => {
      ...
      previewUrl.value = file.type.startsWith("image/")
