<template>
  <main
    class="w-full bg-[rgba(255,255,255,0.06)] shadow-[0_24px_80px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.08)] border border-[rgba(255,255,255,0.12)] rounded-4xl p-8 sm:p-12 text-white font-['Poppins']"
  >
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
      <!-- left panel - config -->
      <div
        class="lg:col-span-5 flex flex-col justify-between min-h-105 gap-y-10"
      >
        <div>
          <h1
            class="text-3xl sm:text-[44px] font-bold leading-tight tracking-tight mb-2"
          >
            Certificate Preview
          </h1>
          <p
            class="text-base sm:text-lg font-normal text-gray-300/90 leading-relaxed mb-8"
          >
          ...
          <div class="flex flex-col gap-y-3">
            ...
            >
              ...
              >
                <div
                  class="grid grid-cols-2 gap-1 text-gray-400 group-hover:text-white mr-5"
                >
                  <div
                    class="w-1.5 h-1.5 bg-current rounded-full"
                    v-for="n in 6"
                    :key="n"
                  ></div>
                </div>
              ...
              <div v-if="isPlaced(variable.key)" class="px-5 pb-5">
                <p class="text-xs text-gray-400 mb-1">
                  {{
                    variable.key === "name"
                      ? "Filled per participant at generation time"
                      : "From Step 1"
                  }}
                </p>
                <p
                  class="text-sm font-medium text-white/90 bg-black/20 border border-white/10 rounded-full px-4 py-2"
                >
                  {{ fieldValues[variable.key] }}
                </p>
                <p
                  v-if="variable.key === 'event_name' && !eventForm.name"
                  class="text-xs text-amber-300 mt-1.5"
                >
                  No event name set yet - go back to Step 1.
                </p>
                <p
                  v-if="variable.key === 'date' && !eventForm.endDate"
                  class="text-xs text-amber-300 mt-1.5"
                >
      ...
      <div class="lg:col-span-7 lg:pl-6">
        <div
          class="w-full aspect-16/10 bg-white rounded-2xl shadow-2xl overflow-hidden"
        >
          <CertificateCanvas
        ...
        <p class="text-xs text-gray-400 mt-3">
          Drag a placed field to reposition it. Positions are saved relative to
          the template so they stay accurate at any canvas size.
        </p>
      </div>
    </div>
  </main>
